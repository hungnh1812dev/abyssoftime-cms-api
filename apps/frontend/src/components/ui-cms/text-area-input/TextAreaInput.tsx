"use client";

import React, { ChangeEvent, ComponentProps } from "react";
import { RegisterOptions, useController, useFormContext } from "react-hook-form";

import { Field, FieldError, FieldLabel } from "@/components/ui/field";
import { Textarea } from "@/components/ui/textarea";
import { cn } from "@/lib/utils";

interface TextAreaInputProps extends Omit<ComponentProps<"textarea">, "defaultValue"> {
  name?: string;
  label?: string;
  rules?: RegisterOptions;
  shouldUnregister?: boolean;
  defaultValue?: string;
  classes?: {
    root?: string;
    label?: string;
    input?: string;
  };
}

const TextAreaInput: React.FC<TextAreaInputProps> = ({ className, classes, name, label, rules, shouldUnregister, defaultValue, disabled, onChange, ...others }) => {
  const { control } = useFormContext();
  const { field, fieldState } = useController({ name: name!, control, rules, shouldUnregister, defaultValue: defaultValue ?? "", disabled });

  return (
    <Field className={cn("flex w-full flex-col items-start gap-1", className, classes?.root)} data-invalid={fieldState.invalid}>
      {label && (
        <FieldLabel className={classes?.label} htmlFor={`text-input-${name}`}>
          {label}
        </FieldLabel>
      )}
      <Textarea
        {...others}
        {...field}
        id={`text-input-${name}`}
        onChange={(e: ChangeEvent<HTMLTextAreaElement>) => {
          field.onChange(e);
          onChange?.(e);
        }}
        className={classes?.input}
        aria-invalid={fieldState.invalid}
      />
      {fieldState.error && <FieldError errors={[fieldState.error]} />}
    </Field>
  );
};

export default TextAreaInput;
