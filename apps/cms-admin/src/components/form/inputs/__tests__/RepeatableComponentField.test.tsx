...
describe("RepeatableComponentField — collapsible entries", () => {
  ...
  it("hint text shows the flagged header field instead of the first text field", async () => {
    const schema: FieldDefinition[] = [
      {
        name: "skills",
        type: "component",
        ...
        fields: [
          { name: "category", type: "text" },
          { name: "skill", type: "text", header: true },
        ],
      },
    ];
    renderWithProviders(<ContentTypeBuilder schema={schema} mutationFn={noop} />);
    ...
    await userEvent.type(screen.getByLabelText("skill"), "React");
  ...
  it("uses the last flagged field when multiple fields are flagged as header", async () => {
    const schema: FieldDefinition[] = [
      {
        name: "skills",
        type: "component",
        repeatable: true,
        fields: [
          { name: "category", type: "text", header: true },
          { name: "skill", type: "text", header: true },
        ],
      },
    ];
  ...
  it("falls back to the first text field when the flagged field is not type text", async () => {
    const schema: FieldDefinition[] = [
      {
        name: "skills",
        type: "component",
        ...
        fields: [
          { name: "category", type: "text" },
          { name: "level", type: "number", header: true },
        ],
      },
    ];
    renderWithProviders(<ContentTypeBuilder schema={schema} mutationFn={noop} />);
  ...
  it("collapsed entry card has reduced padding and no header margin", async () => {
    ...
    const toggle = screen.getByRole("button", { expanded: false });
    const card = toggle.closest(".rounded-md.border") as HTMLElement;
    const headerRow = toggle.parentElement as HTMLElement;

    expect(card).toHaveClass("p-2");
    expect(card).not.toHaveClass("p-4");
    expect(headerRow).not.toHaveClass("mb-3");
  });
  ...
  it("expanded entry card keeps the original padding and header margin", async () => {
    ...
    expect(headerRow).toHaveClass("mb-3");
  });
