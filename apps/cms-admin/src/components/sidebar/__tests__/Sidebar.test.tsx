...
const mockUseAuth = vi.fn(() => ({ role: 'super_admin', displayName: 'Jane Admin', permissions: ALL_SETTINGS_PERMISSIONS, token: 'x', userId: '1', loading: false, login: vi.fn(), logout: vi.fn() }));
...
beforeEach(() => {
  localStorage.clear();
  mockUseAuth.mockReturnValue({ role: 'super_admin', displayName: 'Jane Admin', permissions: ALL_SETTINGS_PERMISSIONS, token: 'x', userId: '1', loading: false, login: vi.fn(), logout: vi.fn() });
});
