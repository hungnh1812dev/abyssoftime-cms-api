...
See `/docs/ENTRYPOINT.md` for the full doc index. This file only orients — module details live in `docs/documents/*` and must be removed from here once fully captured there (per `docs/rules/workflow.md`'s Root docs rule).
...
Lint:    bun run lint        # always this — never `bunx eslint .` directly (see docs/rules/workflow.md)
