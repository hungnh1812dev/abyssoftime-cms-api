...
Three read queries against `/api/content-types` (list, by-id, by-slug) plus `useUpdateListFields` (`PATCH /api/content-types/:slug/list-fields`) — the one schema-adjacent thing this frontend can mutate; everything else about a content type's schema is read-only here (defined server-side as schema-as-code, per the API's `content-type.md`). `useUpdateListFields` backs the "configure columns" flow in [documents.md](./documents.md)'s `ColumnChooserDialog`.
...
- **Locale resolution** — `activeLocale` defaults to the locale marked `isDefault` (falling back to the first locale in the list), overridable via the `LocaleSelector` in the header actions. Switching locale while the form is dirty opens a "Discard unsaved changes?" confirmation dialog (`pendingLocale` staging state) instead of switching immediately.
...
- **Audit line** — `doc.data.updatedByName`/`updatedAt`, formatted via a local `formatAuditDate` (relative for <24h, absolute `Intl.DateTimeFormat` otherwise) — not shared with [documents.md](./documents.md)'s near-identical `formatDate` in `CollectionListPage`, a small duplication worth consolidating if a third caller appears.
...
- `media` fields render `MediaInput` directly (not wrapped in `FormField` — it self-registers via `Controller`).
