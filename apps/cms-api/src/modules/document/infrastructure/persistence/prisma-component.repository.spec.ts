...
describe("PrismaComponentRepository", () => {
  ...
  describe("upsertAll", () => {
    it("deletes existing rows scoped to the parent, then bulk-inserts the new components in order", async () => {
      const components = [
      ...
      expect(String(deleteSql)).toContain("parent_component_id IS NOT DISTINCT FROM $3");
      expect(deleteParams).toEqual(["doc-1", "draft", null]);
    ...
    it("scopes the delete to a non-null parentComponentId using the same placeholder semantics", async () => {
      await repository.upsertAll("cv-page", ["experience", "role"], "doc-1", "draft", "comp-1", [], FIELDS);

      const [deleteSql, ...deleteParams] = prisma.$executeRawUnsafe.mock.calls[0];
      expect(String(deleteSql)).toContain('DELETE FROM "components_cv_page__experience_role"');
      expect(deleteParams).toEqual(["doc-1", "draft", "comp-1"]);
    });

    it("does not issue an INSERT when the components array is empty", async () => {
      await repository.upsertAll("cv-page", ["experience"], "doc-1", "draft", null, [], FIELDS);
