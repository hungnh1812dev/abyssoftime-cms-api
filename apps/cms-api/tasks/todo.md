...
- [x] Task 1.2 — e2e: flip every tokenless read `gql()` call to use `readScopedApiToken`; add a missing-token-rejects negative case per read family
- [x] **Checkpoint 1 (final):** all checks green; docs updated; `SPEC.md` trimmed to pointer — commit — feature complete
