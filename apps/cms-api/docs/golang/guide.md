...
The CMS exposes a GraphQL endpoint alongside REST — not a replacement for it. Unlike REST, the schema is **generated at build time** per content-type (`make graphql-generate`), not resolved dynamically per request — every collection type gets its own typed `Query`, `Mutation`, `Filter`, and `OrderBy` fields.

> Porting this pagination/filter/order/response contract to another stack? See [graphql-list-api-spec.md](graphql-list-api-spec.md) for the full framework-agnostic spec (exact validation rules, error strings, and edge cases).
...
curl -X POST http://localhost:8080/graphql \
  ...
  -d '{"query": "{ cvPage(documentId: \"abc\", locale: \"en\") { position company } }"}'
```

### Schema shape

For a **collection** content type (e.g. `cv-page`, slug → PascalCase `CvPage`), codegen emits:

```graphql
extend type Query {
  cvPage(documentId: ID!, locale: String, status: String): CvPage
  cvPages(pagination: PaginationInput, filters: [CvPageFilter!], orderBy: CvPageOrderBy, locale: String, status: String): CvPageList!
}
```

- Single-item query: `<camelSlug>` (e.g. `cvPage`).
- List query: `<camelSlug>` **pluralized** (e.g. `cvPages`, `enItVocabs`) — not `<slug>List`; that name is reserved for the wrapper *type* (`CvPageList`), not the query field.

For a **single** content type (e.g. `cv-contact`, no list, no `documentId`):
...
`Query` operations are public by default and return **published** data only. Passing `status: "draft"` opts into draft data, but only takes effect when the request carries a valid JWT/access token (`middleware.UserID(ctx) != ""`) — otherwise it silently falls back to published, same as omitting `status`. `Mutation` operations always require auth via the `@auth` directive.

### Pagination

`PaginationInput` supports two mutually exclusive modes — mixing both is a resolver-level error:

```graphql
input PaginationInput {
  start: Int      # offset mode
  limit: Int      # offset mode — -1 means "no limit, return all"
...
| Rule | Behavior |
|---|---|
| No input | Defaults to `start: 0, limit: 10` |
| Offset mode (`start`/`limit`) | `start` clamps to `0` if negative; `limit: 0` is an error; `limit: -1` returns every match; `limit` otherwise caps at `100` |
| Page mode (`page`/`pageSize`) | Both fields required together; `page < 1` is an error; `pageSize: 0` is an error; `pageSize` caps at `100` |
| Both offset and page fields set | Error: `"cannot mix offset (start/limit) and page (page/pageSize) modes"` |

Page mode is converted internally to offset mode: `start = (page - 1) * pageSize`.

### Filtering

`filters` is an array of `<Type>Filter!` — array entries are implicitly **ANDed**. Each filter input combines per-field operators with `and`/`or`/`not` combinators:
...
input CvPageFilter {
  ...
  position: StringFilter   # one entry per content field
...
Base filter types and their operators:

| Filter type | Operators | Used for |
|---|---|---|
| `IDFilter` | `eq`, `ne`, `in`, `notIn` | `documentId` |
| `StringFilter` | `eq`, `ne`, `in`, `notIn` | `text`, `richtext` |
| `NumberFilter` | `eq`, `ne`, `in`, `notIn` | `number` |
| `BooleanFilter` | `eq`, `ne` | `boolean` |
| `TimeFilter` | `eq`, `ne` | `createdAt`, `updatedAt`, `publishedAt` |

`component`, `media`, and `json` fields are **excluded** from `<Type>Filter` entirely — they cannot be filtered on.

Example — company is "Acme" OR position contains a match from a set:

```graphql
{
  cvPages(filters: [{ company: { eq: "Acme" } }, { or: [{ position: { in: ["Backend Engineer", "Tech Lead"] } }] }]) {
...
`orderBy` is a typed `<Type>OrderBy` struct — every field is optional, but only the **first non-nil field** (in schema declaration order) is applied; the rest are ignored:
...
If `orderBy` is omitted (or every field is nil), results default to `createdAt DESC`.

### Response shape — items + metadata

Every list query returns a `<Type>List!` wrapper, never a bare array, so pagination metadata always travels with the results:
...
type PaginationMeta {
  page: Int!       # 1-indexed page number, derived from start/limit
  pageSize: Int!   # effective page size (equals `total` when limit: -1)
  total: Int!      # total matching documents, independent of pagination
}
```
...
curl -X POST http://localhost:8080/graphql \
  -H 'Content-Type: application/json' \
  -d '{"query": "{ cvPages(pagination: { page: 1, pageSize: 2 }, orderBy: { createdAt: DESC }) { items { documentId position company } meta { pagination { page pageSize total } } } }"}'
```
...
```json
{
  "data": {
    "cvPages": {
      "items": [
        { "documentId": "665f1a...", "position": "Tech Lead", "company": "Acme" },
        { "documentId": "665f1b...", "position": "Backend Engineer", "company": "Acme" }
      ],
      "meta": {
        "pagination": { "page": 1, "pageSize": 2, "total": 7 }
      }
    }
  }
}
...
Resolvers call the existing usecases (`document`, `content_type`, `media`) — no business logic is duplicated. This means the same validation, locale checks, and draft/publish semantics apply to both REST and GraphQL paths. `filters`/`orderBy` conversion from gqlgen typed structs to `entity.FilterNode` happens via reflection in `apps/api/graphql/resolver/filter.go` — see `rules/graphql.md` for the full contract.

### Regenerating after schema changes

Edit the relevant `content-types/*.json` file, then:

```sh
make graphql-generate
```

This regenerates `apps/api/graphql/schema/*.graphql`, `graphql/generated/`, and `graphql/resolver/content_gen.go`. All of these are code-generated and never hand-edited.
...
| `TRUSTED_PROXIES` | Comma-separated CIDRs of reverse proxies to trust for `X-Forwarded-For` client-IP resolution (rate limiting keys on this). Empty means trust none — the raw connection address is used. **Required on Render/any platform behind a reverse proxy**, or every request behind that proxy shares one rate-limit bucket. | *(empty — trust none)* |
...
The project includes a `docker-compose.yml` that runs the full stack: API, web frontend, PostgreSQL (default `DB_DRIVER`), and MongoDB.
...
**Set `TRUSTED_PROXIES` on `cms-api`:** Render sits in front of the API as a
reverse proxy, so without this set every client's requests are seen as
coming from the same address, collapsing everyone onto one shared rate-limit
bucket — right after a cold-start wake, that can 429 a legitimate user's
session refresh. Render doesn't publish a fixed CIDR for its edge proxy, so
set:
...
This is safe here specifically because a Render Web Service is never
reachable except through Render's own routing layer — there's no direct
path for a client to spoof `X-Forwarded-For` themselves. (Don't reuse this
value for a self-hosted/VPS deploy where the app might be directly exposed —
there, set it to your actual reverse-proxy container's CIDR instead. See the
`TRUSTED_PROXIES` row in [Config reference](#config-reference).)
