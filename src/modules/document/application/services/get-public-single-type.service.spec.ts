...
import { GetPublicSingleTypeService } from "./get-public-single-type.service";

describe("GetPublicSingleTypeService", () => {
  const FIELDS: FieldDefinition[] = [{ name: "position", type: "text" }];

  function buildContentType(draftToPublish: boolean): ContentTypeEntity {
    return new ContentTypeEntity("ct-1", "cv-page", "CV Page", "single", draftToPublish, FIELDS, ["position"], new Date(), new Date());
  }
  ...
  it("throws NotFoundException when no published row exists", async () => {
    ...
    const service = new GetPublicSingleTypeService(schemaResolver, documents, componentIo);

    await expect(service.execute("cv-page")).rejects.toThrow(NotFoundException);
  });

  it("reads only the published version and hydrates its components (mode A) — draft is never queried", async () => {
    const contentType = buildContentType(true);
    const { schemaResolver, documents, componentIo } = buildDeps(contentType);
    const published = new DocumentEntity("doc-1", "published", { position: "Engineer" }, new Date(), new Date(), new Date(), "user-1", "user-1", "user-1");
    documents.findSingle.mockResolvedValue(published);
    componentIo.hydrateComponents.mockResolvedValue({ skills: [{ componentId: "skill-1", level: "expert" }] });

    const service = new GetPublicSingleTypeService(schemaResolver, documents, componentIo);
    const doc = await service.execute("cv-page");

    expect(documents.findSingle).toHaveBeenCalledTimes(1);
    expect(documents.findSingle).toHaveBeenCalledWith("cv-page", "published", contentType.fields);
