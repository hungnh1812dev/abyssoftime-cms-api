...
- `buildFilterWhere(filters: ParsedFilter[], paramIndex)` → `null` for an empty array, otherwise each `ParsedFilter` (`{ column, operator, value }`) becomes its own AND'd clause with its own sequential placeholder (unlike `search`'s shared placeholder, since each filter binds a distinct value): `$contains` reuses `escapeSearchValue` for an `ILIKE ... ESCAPE '\'` clause, every other operator (`$eq`/`$ne`/`$gt`/`$gte`/`$lt`/`$lte`) maps straight to its SQL comparator (`=`/`<>`/`>`/`>=`/`<`/`<=`). Every column name is `quoteIdent`ed. `PrismaDocumentRepository.listPaginated` ANDs the result into `whereSql` alongside `search`'s clause, applied identically to both the count query and the data query.
