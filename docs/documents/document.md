...
An unknown field, an unknown `$op`, more than one operator on the same field, an operator illegal for that field's value class, or a value that fails its class's coercion — every one of these throws `400 BadRequestException` at parse time; nothing is ever silently dropped or passed through unvalidated. `filters` and `search` combine as independent ANDed clauses, and multiple filtered fields AND together as well (no OR-across-fields, no per-field multi-value).
