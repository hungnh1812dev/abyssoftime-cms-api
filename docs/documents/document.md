...
- **List route** (`ListDocumentsService.execute()`) resolves the whole page in one batch: after `listPaginated`, it collects the page's unique non-`null` `updatedBy` ids into a `Set`, calls `USER_REPOSITORY.findByIds(ids)` **exactly once** regardless of page size or duplicate ids (including once with an empty array on a zero-row page — `findByIds([])` short-circuits before touching Prisma, see `users.md`), and builds a `documentId → { documentId, name }` map consulted per row. This is the only batched call site in the module; every detail-route site above resolves a single id, so batching there would add complexity without reducing round-trips.
