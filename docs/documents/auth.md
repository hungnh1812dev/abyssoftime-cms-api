...
- **`ConsoleEmailSender` is still the only `IEmailSender`** — swapping in a real transactional-email provider is a deliberate, separate decision (which provider, credentials, deliverability) outside the scope of the auth-hardening fix cycle; re-confirmed still true in `docs/documents/auth-issues-fix.md` #10.
...
Unit tests (Jest, mocked `IUserRepository`/`IRoleRepository`/`IEmailSender`/`JwtTokenService`; no `coverageThreshold` gate is configured in `package.json` — it was dropped project-wide in an earlier cycle — but every file here still carries solid unit coverage in practice) live next to each source file: `register.service.spec.ts`, `verify-otp.service.spec.ts`, `resend-otp.service.spec.ts`, `has-users.service.spec.ts`, `login.service.spec.ts`, `refresh-token.service.spec.ts`, `forgot-password.service.spec.ts`, `reset-password.service.spec.ts`, `console-email.sender.spec.ts`, `auth.controller.spec.ts` (delegation only, plus asserting cookies are set/cleared with the right `httpOnly`/`secure`/`sameSite` options — provides a mocked `ConfigService` so the testing module can instantiate `RateLimitGuard`, referenced via `@UseGuards` on most routes; `auth`'s own routes never use `JwtAuthGuard`/`PermissionsGuard`, since they're the ones establishing identity in the first place). `prisma-user.repository.spec.ts` (in `users`) covers `completeVerification`'s Serializable-transaction branches (first-verifier vs. subsequent, P2034 retry-then-succeed, retry exhaustion, non-conflict rethrow) and `create()`'s `P2002` → `UserAlreadyExistsError` translation, both mocking the Prisma client rather than hitting a real DB.

## Verified state (2026-07-24)

`bun run build`, `bunx tsc --noEmit`, `bunx eslint`, and `bun run test:cov` all pass with zero errors for this module (one pre-existing, unrelated `no-floating-promises` warning on `src/main.ts`'s `bootstrap();` call). This is the state after the auth-hardening fix cycle documented in `docs/documents/auth-issues-fix.md` — see that file for the issue-by-issue rationale.
